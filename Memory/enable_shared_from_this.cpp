...
int main()
{
    ...
    // - 멀티스레딩 로직에서 자기 자신을 다른 구성 요소에 전달하는 경우
    // - 어떠한 이유로 자기 자신을 받는 로직이 필요한 경우
    // - 비동기 작업이나 콜백 등을 통해 객체가 다른 구성 요소와 협력해야 하는 경우
    // - 한 번의 함수 호출로 서로를 연결해야 하는 경우
    // - 플러그인이나 옵저버 패턴 등을 구현하기 위해 무언가 등록해야 하는 작업이 필요할 경우
    // - 인터페이스를 구성하여 특정 요소만 다른 시스템에 보여주고자 할 경우
    //
    // 자세한 설명은 enable_shared_from_this_details.cpp와 enable_shared_from_this_examples.cpp를 보도록 한다.
