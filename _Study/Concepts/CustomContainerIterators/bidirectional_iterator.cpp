...
// bidirectional_iterator는 forward_iterator의 특성에 양방향 접근이 가능한 반복자에 대한 컨셉이다.
// 해당 제약을 요구하는 ranges 알고리즘이 있다면 맞춰서 구현해야 한다.
template <std::integral T>
class MyIterator
{
public:
    using iterator_category = std::bidirectional_iterator_tag; // 태그 명칭을 알맞게 수정해야 함(안 맞춰주면 에러 발생).
...
int main()
{
    ...
    // serach_n : bidirectional_iterator
    auto ret = std::ranges::reverse(container);
